...
using UnityEngine;
    ...
    public class SliderMechanism : MonoBehaviour
    {
        ...
        float startMouseAxisValue;
        float startSliderAxisValue;
        ...
        public void StartDragging(Vector3 worldHitPoint)
        {
            if (isDragging) return;
            if (playerSystem != null && playerSystem.IsMoving) return;

            isDragging = true;
            startMouseAxisValue = GetMouseAxisValue(worldHitPoint);
        ...
        void Update()
        {
            ...
            if (dragPlane.Raycast(ray, out float enter))
            {
                ...
                float currentMouseAxis = GetMouseAxisValue(hitPoint);
                float delta = currentMouseAxis - startMouseAxisValue;
        ...
        float GetMouseAxisValue(Vector3 worldPoint)
        {
            Vector3 local = transform.parent != null
                ? transform.parent.InverseTransformPoint(worldPoint)
                : worldPoint;

            return slideAxis == SlideAxis.X ? local.x : local.z;
        }
